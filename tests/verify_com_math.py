...
def test_test_pose_27(calculator, baseline_com):
    print("\nTest: Test Pose 27 (Right Arm Backward)")
    nodes = create_t_pose_nodes()
    # Right Arm BACKWARD
    # Shoulder height is 130
    # Arm length approx 30, Forearm 30
    ...
    z_shift = com.z - baseline_com.z
    print(f"   Test Pose 27 CoM: SCNVector3({com.x:.3f}, {com.y:.3f}, {com.z:.3f})")
    print(f"   Z Shift from T-Pose: {z_shift:.2f}")

    if z_shift < -0.5:
        print("✅ PASS: CoM shifted backward")
        return True
    else:
        print("❌ FAIL: CoM did not shift backward")
        return False
...
def test_test_pose_6(calculator, baseline_com):
    ...
    # Lower hips and legs (Squat)
    drop_amount = 30.0
    upper_joints = ["mixamorig_Hips", "mixamorig_Spine", "mixamorig_Spine1", "mixamorig_Spine2", "mixamorig_Neck", "mixamorig_Head"]
    for j in upper_joints:
        nodes[j].position.y -= drop_amount

    # Arms up (Touchdown relative to dropped shoulders)
    lift_amount = 50.0
    arm_joints = ["mixamorig_RightArm", "mixamorig_RightForeArm", "mixamorig_RightHand", "mixamorig_LeftArm", "mixamorig_LeftForeArm", "mixamorig_LeftHand"]
    for j in arm_joints:
        nodes[j].position.y = nodes[j].position.y - drop_amount + lift_amount

    calculator.bind(nodes)
    result = calculator.calculate_detailed_body_com()
    com = result[0]

    print(f"   Test Pose 6 CoM: SCNVector3({com.x:.3f}, {com.y:.3f}, {com.z:.3f})")

    drop_y = baseline_com.y - com.y
    print(f"   Net Vertical Drop from T-Pose: {drop_y:.2f}")

    if 5.0 < drop_y < 15.0:
        print("✅ PASS: CoM dropped by expected amount for Test Pose 6")
        return True
    else:
...
def test_test_pose_19(calculator, baseline_com):
    print("\nTest: Test Pose 19 (Right Arm Forward)")
    nodes = create_t_pose_nodes()
    # Rotate Right Arm -90 around X (forward)
...
def test_test_pose_22(calculator, baseline_com):
    print("\nTest: Test Pose 22 (Both Arms Backward)")
    nodes = create_t_pose_nodes()
    # Rotate Right Arm and Left Arm 90 around X (backward)
    nodes["mixamorig_RightArm"].position = SCNVector3(20, 130, 0)
    nodes["mixamorig_RightForeArm"].position = SCNVector3(20, 130, -30) # -30 Z
    nodes["mixamorig_RightHand"].position = SCNVector3(20, 130, -60) # -30 Z
    if "mixamorig_RightHandMiddle1" in nodes:
    ...
    z_shift = com.z - baseline_com.z
    print(f"   Test Pose 22 CoM: SCNVector3({com.x:.3f}, {com.y:.3f}, {com.z:.3f})")
    print(f"   Z Shift from T-Pose: {z_shift:.2f}")

    if z_shift < -1.0:
        print("✅ PASS: CoM shifted backward for Test Pose 22")
        return True
    else:
        print("❌ FAIL: CoM failed to shift backward for Test Pose 22")
        return False

def test_test_pose_23(calculator, baseline_com):
    print("\nTest: Test Pose 23 (Forward Reach)")
    nodes = create_t_pose_nodes()
    # Rotate Right Arm and Left Arm -90 around X (forward)
    # This requires adding a positive Z-axis offset to the absolute positions of forearm, hand
    nodes["mixamorig_RightArm"].position = SCNVector3(20, 130, 0)
    nodes["mixamorig_RightForeArm"].position = SCNVector3(20, 130, 30) # +30 Z
    nodes["mixamorig_RightHand"].position = SCNVector3(20, 130, 60) # +30 Z
    if "mixamorig_RightHandMiddle1" in nodes:
    ...
    z_shift = com.z - baseline_com.z
    print(f"   Test Pose 23 CoM: SCNVector3({com.x:.3f}, {com.y:.3f}, {com.z:.3f})")
    print(f"   Z Shift from T-Pose: {z_shift:.2f}")

    if z_shift > 1.0:
        print("✅ PASS: CoM shifted forward for Test Pose 23")
        return True
    else:
        print("❌ FAIL: CoM did not shift forward as expected.")
        return False


def test_test_pose_24(calculator, baseline_com):
    print("\nTest: Test Pose 24 (Right Arm Up)")
    nodes = create_t_pose_nodes()
    # Raise Right Arm: Rotate 180 deg around shoulder to point UP
    # Shoulder height is 130
    # Arm length approx 30 (20->50), Forearm 30 (50->80)
    ...
    nodes["mixamorig_RightHand"].position = SCNVector3(20, 190, 0) # +30 Y
    if "mixamorig_RightHandMiddle1" in nodes:
        nodes["mixamorig_RightHandMiddle1"].position = SCNVector3(20, 200, 0) # +10 Y
    ...
    y_shift = com.y - baseline_com.y
    print(f"   Test Pose 24 CoM: SCNVector3({com.x:.3f}, {com.y:.3f}, {com.z:.3f})")
    print(f"   Y Shift from T-Pose: {y_shift:.2f}")

    if y_shift > 0.5:
        print("✅ PASS: CoM shifted up for Test Pose 24")
        return True
    else:
        print("❌ FAIL: CoM failed to shift up for Test Pose 24")
        return False


def test_test_pose_28(calculator, baseline_com):
    print("\nTest: Test Pose 28 (Both Arms Up)")
    nodes = create_t_pose_nodes()
    # Raise Both Arms

    # Right Arm UP
    nodes["mixamorig_RightArm"].position = SCNVector3(20, 130, 0)
    nodes["mixamorig_RightForeArm"].position = SCNVector3(20, 160, 0)
    nodes["mixamorig_RightHand"].position = SCNVector3(20, 190, 0)
    if "mixamorig_RightHandMiddle1" in nodes:
        nodes["mixamorig_RightHandMiddle1"].position = SCNVector3(20, 200, 0)
    ...
    y_shift = com.y - baseline_com.y
    print(f"   Test Pose 28 CoM: SCNVector3({com.x:.3f}, {com.y:.3f}, {com.z:.3f})")
    print(f"   Y Shift from T-Pose: {y_shift:.2f}")

    if y_shift <= 0.05:
        print("   ❌ FAILED: CoM should shift UP (+Y) significantly.")
        return False

    print("   ✅ PASSED: CoM shifted up correctly.")
    return True

def test_test_pose_29(calculator, baseline_com):
    print("\nTest: Test Pose 29 (Right Leg Forward)")
    nodes = create_t_pose_nodes()
    # Right Leg Forward
    # Shift Right Up Leg, Leg, Foot, and Toe Base forward in Z
    ...
    z_shift = com.z - baseline_com.z
    print(f"   Test Pose 29 CoM: SCNVector3({com.x:.3f}, {com.y:.3f}, {com.z:.3f})")
    print(f"   Z Shift from T-Pose: {z_shift:.2f}")

    if z_shift <= 0.05:
        print("   ❌ FAILED: CoM should shift FORWARD (+Z) significantly.")
        return False

    print("   ✅ PASS: Test Pose 29 shifted CoM FORWARD (+Z)")
    return True


def test_test_pose_30(calculator, baseline_com):
    print("\nTest: Test Pose 30 (Left Leg Forward)")
    nodes = create_t_pose_nodes()
    # Left Leg Forward
    # Shift Left Up Leg, Leg, Foot, and Toe Base forward in Z

    nodes["mixamorig_LeftUpLeg"].position = SCNVector3(-10, 95, 0)
    nodes["mixamorig_LeftLeg"].position = SCNVector3(-10, 95, 30)
    nodes["mixamorig_LeftFoot"].position = SCNVector3(-10, 95, 60)
    if "mixamorig_LeftToeBase" in nodes:
        nodes["mixamorig_LeftToeBase"].position = SCNVector3(-10, 95, 70)
    ...
    z_shift = com.z - baseline_com.z
    print(f"   Test Pose 30 CoM: SCNVector3({com.x:.3f}, {com.y:.3f}, {com.z:.3f})")
    print(f"   Z Shift from T-Pose: {z_shift:.2f}")
    ...
    print("   ✅ PASS: Test Pose 30 shifted CoM FORWARD (+Z)")
    return True
...
def test_test_pose_31(calculator, baseline_com):
    ...
    if z_shift >= -0.05:
        print(f"   ❌ FAILED: Expected CoM to shift backward (negative Z), but shift was {z_shift:.2f}")
        return False
...
def run_verification():
    ...
    if not test_test_pose_22(calculator, t_pose_com):
        sys.exit(1)

    if not test_test_pose_23(calculator, t_pose_com):
        sys.exit(1)
    ...
    if not test_test_pose_27(calculator, t_pose_com):
        sys.exit(1)

    if not test_test_pose_28(calculator, t_pose_com):
        sys.exit(1)

    if not test_test_pose_29(calculator, t_pose_com):
        sys.exit(1)
