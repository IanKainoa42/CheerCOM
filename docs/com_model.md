...
The CoM calculator uses a **17-segment model** based on anthropometric data from Winter (2009) and de Leva (1996). It calculates the total body center of mass as the mass-weighted average of the individual segment centers of mass. This document serves as the primary baseline reference outlining the 17-segment model and its core underlying assumptions, which ensures accurate biomechanical analysis across deterministic poses.
...
Transforms in the 3D body model must be applied in the explicit order of local -> parent -> world coordinates to ensure segment COM points correctly follow bones during motion.
...
3.  The app will cycle through standard poses:
    ...
    *   **Test Pose 26**: Left Arm Backward. Tests one-arm backward Z-axis shift. CoM should shift backward (-Z direction).
    * **Test Pose 27**: Right Arm Backward. Tests one-arm backward Z-axis shift on the right side. CoM should shift backward (-Z direction).
    * **Test Pose 28**: Both Arms Up. Tests vertical shift. CoM should shift up (+Y direction).
    * **Test Pose 29**: Right Leg Forward. Tests one-leg forward Z-axis shift on the right side. CoM should shift forward (+Z direction).
    * **Test Pose 30**: Left Leg Forward. Tests one-leg forward Z-axis shift on the left side. CoM should shift forward (+Z direction).
    * **Test Pose 31**: Right Leg Backward. Tests backward Z-axis shift when the right leg is extended backward. CoM should shift backward (-Z direction).
