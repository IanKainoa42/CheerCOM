...
**Note:** Auto-closing via branch-name coincidence (e.g. issues closing when branches containing their ID are merged or pushed) is not a real fix to prevent daily generation of these open-ended tasks. The session must be cancelled directly at jules.google.com.
